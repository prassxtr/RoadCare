    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button class="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2.5 text-sm font-semibold rounded-xl shadow-sm transition-all self-start sm:self-center">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" class="w-4 h-4">
                <path stroke-linecap="round" stroke-linejoin="round" d="M19 7.5v3m0 0v3m0-3h3m-3 0h-3m-2.25-4.125a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zM4 19.235v-.11a6.375 6.375 0 0112.75 0v.109A12.318 12.318 0 2110.374 21c-2.331 0-4.512-.645-6.374-1.766z" />
            </svg>
            Tambah User Baru
        </button>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div class="bg-slate-50 border border-slate-200 rounded-xl p-4 flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-lg font-bold text-slate-800">{{ $users->where('role', 'admin')->count() }} Akun</p>
            </div>
        </div>
        <div class="bg-slate-50 border border-slate-200 rounded-xl p-4 flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-lg font-bold text-slate-800">{{ $users->where('role', 'petugas')->count() }} Akun</p>
            </div>
        </div>
        <div class="bg-slate-50 border border-slate-200 rounded-xl p-4 flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-lg font-bold text-slate-800">{{ $users->where('role', 'user')->count() }} Akun</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td class="py-4 px-6">
                            ...
                            @elseif($user->role == 'petugas')
                                <span class="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-emerald-50 text-emerald-700 border border-emerald-100">
                                    Petugas Lapangan
                                </span>
                            @else
                                <span class="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-purple-50 text-purple-700 border border-purple-100">
                                    Masyarakat
                                </span>
                            @endif
                        </td>
                ...
                @empty
                    <tr>
                        <td colspan="5" class="py-8 text-center text-slate-400">Belum ada data pengguna.</td>
                    </tr>
                @endforelse
                </tbody>
            </table>
